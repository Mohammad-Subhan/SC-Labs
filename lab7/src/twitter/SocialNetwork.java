...
public class SocialNetwork {
		...
		for (Tweet tweet : tweets) {
			...
			for (String word : words) {
				if (word.startsWith("@") && word.length() > 1) {
					String mentioned = word.substring(1).toLowerCase();
			...
			if (!mentionedUsers.isEmpty()) {
				...
				graph.get(author).addAll(mentionedUsers);
			}
	...
	public static List<String> influencers(Map<String, Set<String>> followsGraph) {
		...
		// count how many times each person is mentioned (followed)
		for (Set<String> followed : followsGraph.values()) {
			for (String person : followed) {
				if (followerCount.containsKey(person)) {
					followerCount.put(person, followerCount.get(person) + 1);
				} else {
					followerCount.put(person, 1);
				}
			}
		}
		...
		influencerList.sort((a, b) -> followerCount.get(b) - followerCount.get(a));
